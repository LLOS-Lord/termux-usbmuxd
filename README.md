...
-   **`idevicepair pair` says "No device found." right after Start printed the UDID**: Start verifies the device through the **Unix socket**, while a plain `idevicepair` inherits `USBMUXD_SOCKET_ADDRESS=127.0.0.1:27015` and goes through the **socat TCP relay** - when that relay is dead the device list comes back empty even though everything else works. Menu options 3/4 and `termux-usbmuxd pair/info` now call the tools against the Unix socket directly, so pairing no longer depends on the relay. Standalone Rust `idevice-tools` still need it: `termux-usbmuxd doctor` verifies the relay end-to-end with a real usbmuxd `ReadBUID` exchange (a listening TCP port alone proves nothing - socat accepts the connection first and fails later when its unix-side connect dies), and Start automatically restarts the socat forwarder when the probe fails.
-   **`[ERROR] usbmuxd is not running - 'idevicepair' would just say 'No device found'` immediately after Start printed the UDID** (fixed): the old pre-check was `pgrep -x usbmuxd`, but the daemon is exec'd as **`usbmuxd_proxy`** under `termux-usb -E -e ...`, so its process name never matches `usbmuxd` and Pair refused to run even though the socket was alive. Detection now asks the socket itself (a `ReadBUID` exchange over the Unix socket via `socat`, with an `idevice_id` fallback), which is independent of any process name.
-   **`pkill ... Bad system call` during Start, then "usbmuxd is up, but idevice_id -l shows no device yet"** (fixed): on many Android devices the seccomp profile kills procps `pkill` with SIGSYS the moment it signals a process, so the previous session's `usbmuxd_proxy`/`termux-usb` wrapper survived cleanup and kept the old USB file descriptor claimed - the new daemon could not attach the device. Process handling now walks `/proc` directly and signals with the bash builtin `kill` (classic `kill(2)`, not affected by that seccomp filter), waits for exit, and escalates to SIGKILL for anything that ignores TERM.
-   **No "Trust This Computer" prompt on the iPhone**: iOS shows the trust/pairing popup only when a client actually requests pairing (`idevicepair pair` = menu option **3**), not while usbmuxd merely starts - and never while a stale daemon is holding a dead USB session. The sequence that reliably triggers it: unplug/replug the cable, run Start, then **3) Pair** with the iPhone unlocked; tap Trust and enter the passcode when the popup appears. If nothing appears at all, the cable/OTG link is not passing data or the Termux USB permission popup was dismissed - run Start again and watch for that popup first.
